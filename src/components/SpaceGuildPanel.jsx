...
export default function SpaceGuildPanel({
  commandShip, onClose, onSendToGuild, onSellAtGuild, onReturnFromGuild,
  playerResources, teamGold,
}) {
  ...
  const gc = upgrades.guildConvoy || null
  ...
  return (
    <div className="p-3 rounded" style={{ backgroundColor: '#161b22', border: '1px solid #2a3140' }}>
      ...
      <div className="text-[10px] mb-3" style={{ color: '#8b949e' }}>
        Trade station. Send a convoy to sell resources and units for gold.
      </div>
      ...
      {gc && gc.inTransit && (
        <div className="p-2 rounded mb-2" style={{ backgroundColor: '#0d1117', border: '1px solid #2a3140' }}>
          <div className="flex items-center justify-between">
            <span className="text-[10px] font-semibold" style={{ color: '#d29922' }}>In Transit</span>
      ...
      {gc && !gc.inTransit && (
        <div className="p-2 rounded mb-2" style={{ backgroundColor: '#0d1117', border: '1px solid #6cb4e640' }}>
          <div className="text-[10px] font-semibold mb-1.5" style={{ color: '#3fb950' }}>Docked at Guild</div>
            ...
            <div className="mb-2">
              ...
              {gc.units.map((u, idx) => (
                <div key={idx} className="flex items-center justify-between p-1 rounded mb-0.5"
                  ...
                  <button
                    onClick={async () => {
                      const earned = await onSellAtGuild(commandShip.id, [idx], null)
            ...
            <div className="mb-2">
              ...
              {Object.entries(gc.cargo.resources).filter(([, v]) => v > 0).map(([key, amount]) => (
                <div key={key} className="flex items-center justify-between p-1 rounded mb-0.5"
                  ...
                  <button
                    onClick={async () => {
                      const earned = await onSellAtGuild(commandShip.id, null, { [key]: amount })
                      if (earned) setSellResult(`+${earned}g`)
                      setTimeout(() => setSellResult(null), 2000)
                    }}
                    className="text-[8px] px-1.5 py-0.5 rounded cursor-pointer"
          ...
          <button
            onClick={() => onReturnFromGuild(commandShip.id)}
