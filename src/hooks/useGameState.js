...
export const GROUND_ORES = {
  ...
  diamond:     { id: 'diamond',     name: 'Diamond',     chance: 0.30, minAmt: 1, maxAmt: 1, color: '#b9f2ff', deep: true },
  ...
  async function fireMissile(shipId, missileType, targetRow, targetCol, targetBoard, warheadType) {
    ...
    if (board === 'ground') {
      ...
      if (warheadType) {
        ...
        let seed = (targetRow * 7919 + targetCol * 104729 + Date.now()) & 0x7fffffff
        const craterRand = () => { seed = (seed * 1664525 + 1013904223) & 0x7fffffff; return seed / 0x7fffffff }

        const craterTiles = []
        for (const tile of groundTiles) {
          const d = hexDistance(targetRow, targetCol, tile.grid_row, tile.grid_col)
